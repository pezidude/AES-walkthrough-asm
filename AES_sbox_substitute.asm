; Procedure to substitute byte in AL via AES S-Box
; Input: AL = byte to substitute
; Output: AL = substituted byte via S-Box
...
section .text
global substitute_al_via_sbox

;----------------------------------
; substitute_al_via_sbox:
;   AL = input byte
;   returns AL = sbox[AL]
;----------------------------------
substitute_al_via_sbox:
    ...
    ; AL is input; zero-extend AL into RCX to get index
    xor rcx, rcx
    mov cl, al

    ; load byte from sbox into DL
    lea rax, [rel sbox]
    mov dl, [rax + rcx]

    ; return result in AL (DL -> AL)
    mov al, dl
